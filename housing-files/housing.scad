    ...
    color("LightGrey") {
        ...
        difference() {
            translate([length/2+40, width/2-10, wall])
                rotate([0, 0, 90]) pcb_base(HW803_1WAY_RELAY, 10, 5, 3);
            translate([length/2+40, width/2-10, wall])
                rotate([0, 0, 90]) pcb_screw_positions(HW803_1WAY_RELAY) cylinder(d=5, h=10);
        }
        ...
        for (i = [13.5:4:34]) {
            translate([length-i, width/2+71.25, height/2-6]) rotate([0, 0, 0])
                cube([wall/2, 3*wall, height*0.8], center=true);
            translate([length-i, width/2-71.25, height/2-6]) rotate([0, 0, 0])
                cube([wall/2, 3*wall, height*0.8], center=true);
        }
        // dividing wall slot
...
module housing() {
    ...
    translate([length/2+40, width/2-10, wall+10])
        rotate([0, 0, 90]) pcb_assembly(HW803_1WAY_RELAY, 0, 5);
}
...
module housing_cover_base() {
    color("Grey") {
        difference() {
            ...
            // Panel text
            translate([length / 2, width / 2 -26, cover_height_brim]) TouchButtomSymbol(r=5, h=0.8);
            translate([length / 2, width - 25, cover_height_brim - 0.4]) cube([100, 0.4, 1], center=true);
        ...
        // LCD PCB mount
        difference() {
            translate([length / 2, width / 2, cover_height_brim - wall])
                rotate([180, 0, 0])
                    pcb_base(LCD2004APCB, display_thickness(LCD2004A)-2, 0, 2.5);
            translate([length / 2, width / 2, cover_height_brim - wall + 2])
                rotate([180, 0, 0]) pcb_screw_positions(LCD2004APCB) cylinder(d=5, h=10);
        }
        ...
        difference() {
            translate([length / 2 - 72, width / 2, cover_height_brim - wall])
                rotate([180, 0, 90]) pcb_base(PERF60x40, 6, 0, 2);
            translate([length / 2 - 72, width / 2, cover_height_brim - wall + 2])
                rotate([180, 0, 90]) pcb_screw_positions(PERF60x40) cylinder(d=3.5, h=10);
        }
        // dividing wall
